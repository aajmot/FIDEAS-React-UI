...
const CreditNoteView: React.FC<CreditNoteViewProps> = ({ noteId, onBack }) => {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const response = await accountExtensions.getCreditNoteById(noteId);
      const noteData = response.data?.data || response.data;
      
      // Fetch customer details
      if (noteData.customer_id) {
        try {
          const customerResponse = await inventoryService.getCustomers({ per_page: 1000 });
          if (customerResponse.data && customerResponse.data.length > 0) {
            const customer = customerResponse.data.find((c: any) => c.id === noteData.customer_id);
            if (customer) {
              noteData.customer_name = customer.name;
              noteData.customer_address = customer.address || '';
              noteData.customer_phone = customer.phone;
              noteData.customer_tax_id = customer.tax_id || customer.gstin || '';
            }
          }
        } catch (error) {
      ...
      // Fetch product details for items and ensure all fields are present
      ...
      showToast('error', 'Failed to load credit note');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="purchase-invoice-content print-container max-w-4xl mx-auto p-8 print:p-3 print:max-w-none">
        ...
        <div className="print-header border-b-2 border-blue-600 pb-3 mb-4 print:pb-2 print:mb-2">
          <div className="flex justify-between items-start">
            ...
            <div className="text-right">
              <h2 className="print-po-title text-2xl font-bold text-gray-800 mb-2">CREDIT NOTE</h2>
        ...
        <div className="print-section grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 print:gap-2 print:mb-2">
          <div className="print-order-details bg-gray-50 p-4 rounded-lg print:p-2">
            <h3 className="text-sm font-semibold text-gray-800 mb-2 border-b border-gray-200 pb-1 print:text-xs print:mb-1">Credit Note Information</h3>
          ...
          <div className="print-order-details bg-blue-50 p-4 rounded-lg print:p-2">
            ...
            <div className="space-y-2 print:space-y-1">
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="flex-1 font-semibold text-gray-900 text-base">{note.customer_name || '-'}</span>
              </div>
              <div className="flex mb-1">
                <span className="font-medium text-gray-600 w-20">Address:</span>
                <span className="flex-1 text-gray-900 whitespace-pre-line text-sm">{note.customer_address || '-'}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="font-medium text-gray-600 w-20">Phone:</span>
                <span className="flex-1 text-gray-900">{note.customer_phone || '-'}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="font-medium text-gray-600 w-20">GSTIN:</span>
                <span className="flex-1 text-gray-900 font-medium">{note.customer_tax_id || '-'}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="print-section mb-4 print:mb-2">
          <h3 className="text-sm font-semibold text-gray-800 mb-2 border-b border-gray-200 pb-1 print:text-xs print:mb-1">Credit Note Items</h3>
        ...
        <div className="print-section grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 print:gap-2 print:mb-2">
          <div>
            ...
            <ul className="text-xs text-gray-600 leading-tight print:text-xs print:leading-none">
              <li>• This credit note is issued for sales return</li>
              <li>• Amount will be adjusted in future invoices</li>
          ...
          <div className="print-summary bg-gradient-to-br from-blue-50 to-blue-100 p-4 rounded-lg shadow-sm border border-blue-200 print:p-2">
            <h4 className="text-sm font-semibold text-gray-800 mb-2 border-b border-blue-200 pb-1 print:text-xs print:mb-1">Credit Note Summary</h4>
            <div className="space-y-1 print:space-y-0">
              <div className="flex justify-between items-center">
