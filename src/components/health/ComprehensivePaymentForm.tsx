...
interface PaymentAllocation {
  ...
  allocated_amount_foreign: number;
...
const ComprehensivePaymentForm: React.FC<ComprehensivePaymentFormProps> = ({ 
  ...
  const loadPatients = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}/api/v1/health/patients`, {
  ...
  const loadInvoices = async (partyId: number) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/api/v1/health/invoices?patient_id=${partyId}&per_page=1000`,
        { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } }
      );
      const data = await response.json();
  ...
  const calculateTotals = () => {
    ...
    const unallocated = totalDetails - totalAllocated;
    
    setFormData(prev => ({
      ...prev,
      total_amount_base: totalDetails,
      allocated_amount_base: totalAllocated,
      unallocated_amount_base: unallocated
    }));
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const paymentData = {
      ...
      details: paymentDetails.map(d => ({
        ...
        payment_gateway: d.payment_gateway || null,
        gateway_transaction_id: d.gateway_transaction_id || null,
        gateway_status: d.gateway_status || null,
        gateway_fee_base: d.gateway_fee_base || 0,
        amount_base: d.amount_base,
        amount_foreign: d.amount_foreign || 0,
      ...
      allocations: allocations.filter(a => a.document_id > 0).map(a => ({
        ...
        allocated_amount_foreign: a.allocated_amount_foreign || 0,
        discount_amount_base: a.discount_amount_base || 0,
        adjustment_amount_base: a.adjustment_amount_base || 0,
        ...
        <form onSubmit={handleSubmit} className="p-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
            ...
            <div>
              ...
              <DatePicker value={formData.payment_date} 
                onChange={(v) => setFormData(prev => ({ ...prev, payment_date: v }))} />
            </div>
            <div>
              ...
              <SearchableDropdown
                ...
                onChange={(v) => setFormData(prev => ({ ...prev, payment_type: v as string }))}
            ...
            <div>
              ...
              <SearchableDropdown
                ...
                onChange={(v) => setFormData(prev => ({ ...prev, status: v as string }))}
                multiple={false} searchable={false} />
            </div>
            <div>
              ...
              <SearchableDropdown
                ...
                onChange={(v) => setFormData(prev => ({ ...prev, party_type: v as string }))}
                multiple={false} searchable={false} />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="number" value={formData.exchange_rate} step="0.0001"
                ...
                <table className="min-w-full text-xs">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-2 py-2 text-left">Instrument Date</th>
                      ...
                      <tr key={detail.id} className="border-t">
                        <td className="px-2 py-2">
                          <SearchableDropdown
                            ...
                            onChange={(v) => updatePaymentDetail(detail.id, 'payment_mode', v)}
                            multiple={false} searchable={false} />
                        </td>
                        <td className="px-2 py-2">
                        ...
                        <td className="px-2 py-2">
                          <SearchableDropdown
                            ...
                            onChange={(v) => updatePaymentDetail(detail.id, 'account_id', Number(v))}
                            placeholder="Select..." multiple={false} searchable={true} />
                        </td>
                        <td className="px-2 py-2">
                          <button type="button" onClick={() => removePaymentDetail(detail.id)}
                      ...
                      <tr key={allocation.id} className="border-t">
                        <td className="px-2 py-2">
                          <SearchableDropdown
                            ...
                            value={allocation.document_type}
                            onChange={(v) => updateAllocation(allocation.id, 'document_type', v)}
                            multiple={false} searchable={false} />
                        </td>
                        <td className="px-2 py-2">
                          <SearchableDropdown
                        ...
                        <td className="px-2 py-2">
                          <button type="button" onClick={() => removeAllocation(allocation.id)}
                            className="text-red-600 hover:text-red-800">
                            <Trash2 className="h-4 w-4" />
                          </button>
                        </td>
