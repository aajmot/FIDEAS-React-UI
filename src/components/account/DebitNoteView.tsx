...
interface DebitNoteViewProps {
  noteId: number;
  onBack: () => void;
}

const DebitNoteView: React.FC<DebitNoteViewProps> = ({ noteId, onBack }) => {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const response = await accountExtensions.getDebitNoteById(noteId);
      const noteData = response.data?.data || response.data;
      
      // Fetch supplier details
      if (noteData.supplier_id) {
        try {
          const supplierResponse = await inventoryService.getSuppliers({ per_page: 1000 });
          if (supplierResponse.data && supplierResponse.data.length > 0) {
            const supplier = supplierResponse.data.find((s: any) => s.id === noteData.supplier_id);
            if (supplier) {
              noteData.supplier_name = supplier.name;
              noteData.supplier_address = supplier.address || '';
              noteData.supplier_phone = supplier.phone;
              noteData.supplier_tax_id = supplier.tax_id || supplier.gstin || '';
            }
          }
        } catch (error) {
      ...
      if (noteData.items && noteData.items.length > 0) {
        try {
          const productsResponse = await inventoryService.getProducts({ per_page: 1000 });
      ...
      noteData.subtotal_base = parseFloat(noteData.subtotal_base) || 0;
      noteData.cgst_amount_base = parseFloat(noteData.cgst_amount_base) || 0;
      noteData.sgst_amount_base = parseFloat(noteData.sgst_amount_base) || 0;
      noteData.igst_amount_base = parseFloat(noteData.igst_amount_base) || 0;
      noteData.ugst_amount_base = parseFloat(noteData.ugst_amount_base) || 0;
      noteData.total_amount_base = parseFloat(noteData.total_amount_base) || 0;
      ...
      showToast('error', 'Failed to load debit note');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="purchase-invoice-view bg-white min-h-screen">
      ...
      <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center print:hidden bg-gray-50">
        <div className="flex items-center">
          ...
          <h2 className="text-lg font-semibold text-gray-900">Debit Note Details</h2>
        </div>
        <button
          onClick={handlePrint}
      ...
      <div className="purchase-invoice-content print-container max-w-4xl mx-auto p-8 print:p-3 print:max-w-none">
        ...
        <div className="print-header border-b-2 border-blue-600 pb-3 mb-4 print:pb-2 print:mb-2">
          <div className="flex justify-between items-start">
            ...
            <div className="text-right">
              <h2 className="print-po-title text-2xl font-bold text-gray-800 mb-2">DEBIT NOTE</h2>
              <div className="text-sm text-gray-600">
                <div>Date: {new Date(note.note_date).toLocaleDateString('en-US', { 
                  year: 'numeric', 
                  month: 'long', 
                  day: 'numeric' 
                })}</div>
        ...
        <div className="print-section grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 print:gap-2 print:mb-2">
          <div className="print-order-details bg-gray-50 p-4 rounded-lg print:p-2">
            <h3 className="text-sm font-semibold text-gray-800 mb-2 border-b border-gray-200 pb-1 print:text-xs print:mb-1">Debit Note Information</h3>
            <div className="space-y-1 print:space-y-0">
              <div className="flex justify-between">
                <span className="font-medium text-gray-600">Note Number:</span>
                <span className="font-semibold text-gray-900">{note.note_number}</span>
              </div>
              <div className="flex justify-between">
                <span className="font-medium text-gray-600">Note Date:</span>
                <span className="text-gray-900">{new Date(note.note_date).toLocaleDateString('en-US', { 
                  year: 'numeric', 
                  month: 'short', 
                  day: '2-digit' 
                })}</span>
              ...
              {note.original_invoice_number && (
                <div className="flex justify-between">
                  <span className="font-medium text-gray-600">Original Invoice:</span>
                  <span className="text-gray-900">{note.original_invoice_number}</span>
                </div>
              )}
              {note.reason && (
                <div className="flex justify-between">
          ...
          <div className="print-order-details bg-blue-50 p-4 rounded-lg print:p-2">
            ...
            <div className="space-y-2 print:space-y-1">
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="flex-1 font-semibold text-gray-900 text-base">{note.supplier_name || '-'}</span>
              </div>
              <div className="flex mb-1">
                <span className="font-medium text-gray-600 w-20">Address:</span>
                <span className="flex-1 text-gray-900 whitespace-pre-line text-sm">{note.supplier_address || '-'}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="font-medium text-gray-600 w-20">Phone:</span>
                <span className="flex-1 text-gray-900">{note.supplier_phone || '-'}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="font-medium text-gray-600 w-20">GSTIN:</span>
                <span className="flex-1 text-gray-900 font-medium">{note.supplier_tax_id || '-'}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="print-section mb-4 print:mb-2">
          <h3 className="text-sm font-semibold text-gray-800 mb-2 border-b border-gray-200 pb-1 print:text-xs print:mb-1">Debit Note Items</h3>
            ...
            <table className="print-table w-full">
              <thead className="bg-gray-100">
                <tr>
                  ...
                  <th className="px-2 py-2 text-center text-xs font-semibold text-gray-700 uppercase tracking-wider border-b">Free</th>
        ...
        <div className="print-section grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 print:gap-2 print:mb-2">
          <div>
            ...
            <ul className="text-xs text-gray-600 leading-tight print:text-xs print:leading-none">
              <li>• This debit note is issued for purchase return</li>
              <li>• Amount will be adjusted in future payments</li>
          ...
          <div className="print-summary bg-gradient-to-br from-blue-50 to-blue-100 p-4 rounded-lg shadow-sm border border-blue-200 print:p-2">
            <h4 className="text-sm font-semibold text-gray-800 mb-2 border-b border-blue-200 pb-1 print:text-xs print:mb-1">Debit Note Summary</h4>
            <div className="space-y-1 print:space-y-0">
              <div className="flex justify-between items-center">
                <span className="text-gray-600">Subtotal:</span>
                <span className="font-medium text-gray-900">{Number(note.subtotal_base || 0).toFixed(2)}</span>
              </div>
              {Number(note.cgst_amount_base || 0) > 0 && (
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">CGST:</span>
                  <span className="font-medium text-gray-900">{Number(note.cgst_amount_base || 0).toFixed(2)}</span>
                </div>
              )}
              {Number(note.sgst_amount_base || 0) > 0 && (
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">SGST:</span>
                  <span className="font-medium text-gray-900">{Number(note.sgst_amount_base || 0).toFixed(2)}</span>
                </div>
              )}
              {Number(note.igst_amount_base || 0) > 0 && (
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">IGST:</span>
                  <span className="font-medium text-gray-900">{Number(note.igst_amount_base || 0).toFixed(2)}</span>
                </div>
              )}
              {Number(note.ugst_amount_base || 0) > 0 && (
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">UGST:</span>
                  <span className="font-medium text-gray-900">{Number(note.ugst_amount_base || 0).toFixed(2)}</span>
                </div>
              )}
              <div className="border-t border-blue-200 pt-3">
                <div className="flex justify-between items-center">
                  <span className="text-lg font-semibold text-gray-800">Total Amount:</span>
                  <span className="print-total text-2xl font-bold text-blue-600">{Number(note.total_amount_base || 0).toFixed(2)}</span>
                </div>
              </div>
            </div>
          </div>
