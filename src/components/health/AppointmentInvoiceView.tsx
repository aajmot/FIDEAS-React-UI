...
interface AppointmentInvoiceViewProps {
  invoice: any;
  onBack: () => void;
}

const AppointmentInvoiceView: React.FC<AppointmentInvoiceViewProps> = ({ invoice, onBack }) => {
    ...
    <div className="report-view bg-white">
      <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center print:hidden bg-gray-50">
        <div className="flex items-center">
          ...
          <h2 className="text-lg font-semibold text-gray-900">Appointment Invoice Details</h2>
        </div>
        <button
          onClick={handlePrint}
      ...
      <div className="report-content report-container max-w-4xl mx-auto p-8">
          ...
          <div className="flex justify-between items-start">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="text-right">
                <h2 className="report-title text-2xl font-bold text-gray-800 mb-2">APT INVOICE</h2>
          ...
          <div className="report-details bg-gray-50 p-4 rounded-lg">
            ...
            <div className="space-y-1">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-gray-900">{formatUTCToLocal(invoice.invoice_date)}</span>
              </div>
              <div className="flex justify-between">
                <span className="font-medium text-gray-600">Due Date:</span>
                <span className="text-gray-900">{formatUTCToLocal(invoice.due_date)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="report-details bg-blue-50 p-4 rounded-lg">
            ...
            <div className="space-y-1">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-gray-900">{invoice.patient_email || '-'}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="report-section grid grid-cols-1 md:grid-cols-1 gap-4 mb-4">
          <div className="report-details bg-green-50 p-4 rounded-lg">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="flex justify-between">
                <span className="font-medium text-gray-600">Name:</span>
                <span className="font-semibold text-gray-900">{invoice.doctor_name || '-'}</span>
              </div>
              <div className="flex justify-between">
                <span className="font-medium text-gray-600">Speciality:</span>
                <span className="text-gray-900">{invoice.doctor_speciality || '-'}</span>
              </div>
              <div className="flex justify-between">
                <span className="font-medium text-gray-600">License:</span>
                <span className="text-gray-900">{invoice.doctor_license_number || '-'}</span>
              </div>
            </div>
          </div>
        </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {invoice.items?.map((item: any, index: number) => (
                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    <td className="px-2 py-2 text-sm text-center text-gray-700 border-b">{item.quantity}</td>
                    <td className="px-2 py-2 text-sm text-right font-medium text-gray-900 border-b">{item.rate}</td>
                    <td className="px-2 py-2 text-sm text-right text-gray-700 border-b">{item.disc_amount}</td>
                    <td className="px-2 py-2 text-sm text-right font-semibold text-gray-900 border-b">{item.total_amount}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="report-section grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div>
            ...
            {invoice.notes && (
              <div className="mt-2">
                <h4 className="font-medium text-gray-800 mb-1 text-xs">Notes:</h4>
          ...
          <div className="report-summary bg-gradient-to-br from-blue-50 to-blue-100 p-4 rounded-lg shadow-sm border border-blue-200">
            ...
            <div className="space-y-1">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-medium text-red-600">-{invoice.discount_amount || 0}</span>
              </div>
              <div className="border-t border-blue-200 pt-3">
                <div className="flex justify-between items-center">
                  <span className="text-lg font-semibold text-gray-800">Total Amount:</span>
                  <span className="report-total text-2xl font-bold text-blue-600">{invoice.final_amount}</span>
                </div>
              </div>
            </div>
          </div>
...
export default AppointmentInvoiceView;