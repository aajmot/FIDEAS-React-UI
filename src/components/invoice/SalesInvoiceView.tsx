...
const SalesInvoiceView: React.FC<SalesInvoiceViewProps> = ({ invoiceId, onBack }) => {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const invoiceResponse = await api.get(`/api/v1/invoice/sales-invoices/${invoiceId}`);
      const invoiceData = invoiceResponse.data?.data || invoiceResponse.data;
      
      // Fetch customer details if we have customer_id
      if (invoiceData.customer_id) {
        try {
          const customerResponse = await inventoryService.getCustomers({ 
            per_page: 1000
          });
          if (customerResponse.data && customerResponse.data.length > 0) {
            const customer = customerResponse.data.find((c: any) => c.id === invoiceData.customer_id);
            
            if (customer) {
              // Update invoice with customer information
              ...
              invoiceData.customer_tax_id = customer.tax_id || customer.gstin || '';
            }
          }
        } catch (error) {
      ...
      // Fetch product details for each item to get product names
      if (invoiceData.items && invoiceData.items.length > 0) {
        try {
          const productsResponse = await inventoryService.getProducts({ per_page: 1000 });
          const products = productsResponse.data || [];
          
          // Map product names to invoice items
      ...
      showToast('error', 'Failed to load sales invoice data');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!invoice) {
    return (
      <div className="text-center py-8">
        <p className="text-gray-500">Sales invoice not found</p>
    ...
    <div className="purchase-invoice-view bg-white min-h-screen">
      ...
      <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center print:hidden bg-gray-50">
        <div className="flex items-center">
          ...
          <h2 className="text-lg font-semibold text-gray-900">Sales Invoice Details</h2>
        </div>
        <button
          onClick={handlePrint}
        ...
        <div className="print-header border-b-2 border-blue-600 pb-3 mb-4 print:pb-2 print:mb-2">
          <div className="flex justify-between items-start">
            ...
            <div className="text-right">
              <h2 className="print-po-title text-2xl font-bold text-gray-800 mb-2">SALES INVOICE</h2>
          ...
          <div className="print-order-details bg-blue-50 p-4 rounded-lg print:p-2">
            ...
            <div className="space-y-2 print:space-y-1">
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="flex-1 font-semibold text-gray-900 text-base">{invoice.customer_name || '-'}</span>
              </div>
              <div className="flex mb-1">
                <span className="font-medium text-gray-600 w-20">Address:</span>
                <span className="flex-1 text-gray-900 whitespace-pre-line text-sm">{invoice.customer_address || '-'}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="font-medium text-gray-600 w-20">Phone:</span>
                <span className="flex-1 text-gray-900">{invoice.customer_phone || '-'}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="font-medium text-gray-600 w-20">GSTIN:</span>
                <span className="flex-1 text-gray-900 font-medium">{invoice.customer_tax_id || '-'}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="print-section grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 print:gap-2 print:mb-2">
          ...
          <div className="print-summary bg-gradient-to-br from-blue-50 to-blue-100 p-4 rounded-lg shadow-sm border border-blue-200 print:p-2">
            ...
            <div className="space-y-1 print:space-y-0">
              ...
              {Number(invoice.discount_amount_base || 0) > 0 && (
                <div className="flex justify-between items-center">
              ...
              {Number(invoice.roundoff || 0) !== 0 && (
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-medium text-gray-900">{Number(invoice.roundoff || 0).toFixed(2)}</span>
                </div>
              )}
